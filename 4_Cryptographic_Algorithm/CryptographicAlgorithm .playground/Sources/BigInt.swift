...
public struct BigInt {
    ...
    public func pow(_ n: BigInt) -> BigInt {
        var result = BigInt("1")
        var i = BigInt("0")
        while i < n {
    ...
    public func nonEqual (_ v: BigInt) -> Bool { !equal(v) }
    ...
    public func rand() -> BigInt {
        let v = self
        var res = BigInt("0")
        var ch = true
        let q = Int.random(in: 0...v.value.count)
        if q == v.value.count {
            for i in v.value {
                if ch {
                    let r = Int.random(in: 0...Int(String(i))!)
                    res = res * BigInt("10") + BigInt("\(r)")
                    if r != Int(String(i)) { ch = false }
                } else {
    ...
    public func gcd(_ v: BigInt) -> BigInt {
        var f = self
        var s = v
        while f != BigInt("0") && s != BigInt("0") {
            if f > s {
                f = f % s
            } else {
                s = s % f
...
public func ^ (first: BigInt, second: BigInt) -> BigInt { return first.pow(second) }
...
public func != (first: BigInt, second: BigInt) -> Bool { return first.nonEqual(second) }
...
public func random (first: BigInt) -> BigInt { return first.rand() }
public func gcd (_ first: BigInt,_ second: BigInt) -> BigInt { return first.gcd(second) }
